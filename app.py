...
def fetch_public_filer_ciks(tickers: List[str]) -> Dict[str, str]:
    ...
    if not ticker_cik_map:
        ...
        if resp.status_code == 200:
            data = resp.json()
            # data.values() are dicts with 'ticker' and 'cik_str'
            for record in data.values():
                tic     = record.get('ticker', '').upper()
                cik_str = record.get('cik_str', '')      # preserve leading zeros
                if tic and cik_str:
...
def fetch_xbrl_data(cik: str) -> Dict[str, Any]:
    ...
    if resp.status_code == 404:
        st.warning(f"No XBRL data found for CIK {cik} (HTTP 404)")
    else:
...
def generate_narrative(df: pd.DataFrame) -> str:
    ...
    if not api_key:
        st.error('OpenAI API key not configured. Set OPENAI_API_KEY in env or Streamlit secrets.')
        return ''
    ...
    system_msg = (
        ...
        'Given a JSON array of company KPI records, write a 120-word summary '
        'highlighting valuation multiples, growth, and margin comparisons.'
    )
...
def main():
    ...
    if st.sidebar.button("Generate Benchmark"):
        ...
        if upload_csv:
            ...
            peers     = df_peers['Ticker'].dropna().astype(str).str.upper().tolist()
        else:
